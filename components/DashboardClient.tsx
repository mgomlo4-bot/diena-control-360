  ...
  useEffect(() => {
    async function loadDashboardData() {
      try {
        ...
        setLoadError(error instanceof Error ? error.message : 'No se pudieron cargar los datos del dashboard.');
      }
    }
    ...
    <section className="space-y-8">
      <div className="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-8 shadow-2xl">
        ...
        <p className="mt-4 max-w-3xl text-slate-300">
          Panel operativo conectado a la capa de datos común. Puede trabajar con localStorage o base de datos según la configuración del sistema.
        </p>
        {loadError ? <p className="mt-4 rounded-xl border border-red-800 bg-red-950/40 p-3 text-sm text-red-100">{loadError}</p> : null}
        <div className="mt-6 flex flex-wrap gap-3">
          <Link href="/cursos" className="inline-flex rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-500">Ir al control de cursos</Link>
          <Link href="/tareas" className="inline-flex rounded-xl border border-slate-700 px-5 py-3 text-sm font-semibold text-slate-200 transition hover:bg-slate-800">Ver tareas operativas</Link>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_380px]">
        <article className="rounded-3xl border border-slate-800 bg-slate-900 shadow-xl">
          <div className="border-b border-slate-800 p-6">
            ...
            <p className="mt-2 text-sm text-slate-400">Tareas generadas desde hitos pendientes y tareas manuales persistidas.</p>
          </div>
              ...
              <div key={task.id} className="grid gap-3 p-5 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {task.courseId ? <Link href={`/cursos/${task.courseId}`} className="text-sm font-semibold text-blue-300 hover:text-blue-200">{task.courseCode}</Link> : null}
                  </div>
        ...
        <aside className="space-y-4">
          <article className="rounded-3xl border border-red-800 bg-red-950/30 p-6 shadow-xl"><p className="text-sm uppercase tracking-[0.3em] text-red-200">Crítico</p><p className="mt-3 text-4xl font-bold">{summary.critical}</p><p className="mt-2 text-sm text-red-100/80">Tareas vencidas o bloqueadas por hitos superados.</p></article>
          <article className="rounded-3xl border border-orange-800 bg-orange-950/30 p-6 shadow-xl"><p className="text-sm uppercase tracking-[0.3em] text-orange-200">Alta prioridad</p><p className="mt-3 text-4xl font-bold">{summary.high}</p><p className="mt-2 text-sm text-orange-100/80">Tareas con vencimiento inmediato.</p></article>
          <article className="rounded-3xl border border-slate-800 bg-slate-900 p-6 shadow-xl"><p className="text-sm uppercase tracking-[0.3em] text-slate-500">Sistema</p><p className="mt-3 text-sm text-slate-300">El dashboard ya usa dataRepository como puerta de entrada a los datos.</p></article>
        </aside>
      </div>
    </section>
  );
