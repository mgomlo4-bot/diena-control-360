...
export function isCourseStatus(status: string): status is CourseStatus {
  return courseStatusCatalog.some((item) => normalizeStatus(item) === normalizeStatus(status));
}
...
export function isMilestoneCompleted(course: Course, milestoneId: string): boolean {
  return Boolean(getMilestone(course, milestoneId)?.completed);
}

export function hasFinalizationChainPending(course: Course): boolean {
  return ['final-news-request', 'acta-remission', 'acta-receive', 'bod-finalization', 'admin-close'].some((id) => isMilestonePending(course, id));
}
