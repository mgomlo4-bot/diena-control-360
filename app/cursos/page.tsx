...
export default function CoursesPage() {
  ...
  const filteredCourses = useMemo(() => applyQuickFilter(searchCourses(courses, query), quickFilter), [courses, query, quickFilter]);
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-4xl text-slate-400">Buscador, filtros rápidos, exportación y seguimiento automático por hitos. Datos gestionados mediante dataRepository.</p>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-xl">
        <div className="grid gap-4 xl:grid-cols-[1fr_auto_auto_auto] xl:items-center">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por código, nombre, estado, escuela, tipo o siguiente hito..." className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-100 outline-none focus:border-blue-500" />
          ...
          <button type="button" onClick={exportVisibleCourses} className="rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-500">Exportar cursos filtrados</button>
