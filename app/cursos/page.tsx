...
import { QuickFilter, applyCourseFilters } from '../../lib/courseSearch';
...
const quickFilters: { id: QuickFilter; label: string }[] = [
  ...
  { id: 'pendienteBod', label: 'Pendiente BOD' },
...
function statusClass(milestone: CourseMilestone, isNext: boolean): string {
  ...
  if (milestone.finalizationChain) return 'border-violet-700 bg-violet-950/30 text-violet-100';
...
export default function CoursesPage() {
  const normalizedMockCourses = useMemo(() => mockCourses.map((course) => ({
    ...course,
    milestones: generateCourseMilestones(course.startDate, course.endDate, course.requiresMedicalAndPhysical, course.milestones),
  })), []);
  const [courses, setCourses] = useState<Course[]>(normalizedMockCourses);
  const [selectedCourseId, setSelectedCourseId] = useState(normalizedMockCourses[0]?.id ?? '');
  ...
  const totals = useMemo(() => ({
    ...
    pendingBod: courses.filter(hasPendingFinalizationPublication).length,
  ...
  const finalizationMilestones = getFinalizationMilestones(selectedCourse.milestones);
  const pendingFinalizationMilestones = getPendingFinalizationMilestones(selectedCourse.milestones);
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-4xl text-slate-400">Filtros por estado operativo, automatización de fechas y cadena posterior a fecha fin para acta, BOD de finalización y cierre.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-6">
          ...
          <Kpi label="Pendiente BOD fin" value={totals.pendingBod} />
        </div>
      </div>

      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-xl">
        <div className="grid gap-4 xl:grid-cols-[1fr_280px_auto_auto] xl:items-end">
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por código, nombre, estado, escuela, tipo, acta o BOD..." className="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-slate-100 outline-none focus:border-blue-500" />
